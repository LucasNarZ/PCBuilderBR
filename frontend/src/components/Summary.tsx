import { CircleCheck, ExternalLink, X, CircleAlert } from "lucide-react";
...
export function Summary() {
    ...
    const totalPrice = Object.values(build).reduce((acc, component) => acc + component.offer.price, 0)

    const openCompleteSetupLinks = () => {
        Object.values(build).forEach((component) => window.open(component.offer.url, '_blank', 'noopener,noreferrer'))
    }
    ...
    return (
        <div className="flex flex-col p-7 rounded-xl bg-card text-card-foreground sticky top-24 border-2 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.1)] overflow-hidden">
            ...
            <p className="font-bold text-xs uppercase tracking-widest text-muted-foreground/60">Análise de Custo</p>
